...
void solve()
{
    ...
    */
    ...
    for (int i = 1; i<=k; i++)
    {
        ...
        if (i!=a)
            kom = min(kom,abs(v[a].ff-v[i].ff)+abs(v[a].ss-v[i].ss));
        if (i!=b)
            kom2 = min(kom2,abs(v[i].ff-v[b].ff)+abs(v[i].ss-v[b].ss));
    ...
    int ans = abs(v[a].ff-v[b].ff)+abs(v[a].ss-v[b].ss);
    ans = min(ans,kom2+kom);
    ...
    for (int i = 1; i<=n; i++)
    {
        ...
        int now = abs(v[a].ff-v[i].ff)+abs(v[a].ss-v[i].ss);
        now += abs(v[i].ff-v[b].ff)+abs(v[i].ss-v[b].ss);
        ans = min(ans,now);
    }
    cout<<ans<<nl;

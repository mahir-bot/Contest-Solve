...
void solve()
{
    ...
    for (int i = 1; i<=p; i++)
    {
        ...
        difference[i] = store[i]-store[i-1];
        prefix[i] =prefix[i-1]+ (os.order_of_key(store[i])*difference[i]);
    }
    ...
    for (int i = p; i>1; i--)
    {
        // cout<<(store[i]-store[i-1])<<nl;
        // cout<<((a-os.order_of_key(store[i-1]))-m[store[i-1]])<<nl;

        // cout<<(store[i]-store[i-1])* ((a-os.order_of_key(store[i-1]))-m[store[i-1]])<<nl<<nl;

        suffix[i] = suffix[i+1] + (store[i]-store[i-1])* ((a-os.order_of_key(store[i-1]))-m[store[i-1]]);
    }

...
int prefixX[3000][3000], prefixY[3000][3000];
int suffixX[3000][3000], suffixY[3000][3000];
void solve()
{
    ...
    for (int i = 0; i < a; i++)
    {
        for (int j = 0; j < b; j++)
        {
            if (v[i][j] == '.')
            {
                prefixX[i][j] = (j > 0 ? prefixX[i][j - 1] : 0) + 1;
            }
    ...
    for (int i = a - 1; i >= 0; i--)
    {
        for (int j = b - 1; j >= 0; j--)
        {
            if (v[i][j] == '.')
            {
                suffixX[i][j] = (j + 1 < b ? suffixX[i][j + 1] : 0) + 1;
            }
    ...
    for (int j = 0; j < b; j++)
    {
        for (int i = 0; i < a; i++)
        {
            if (v[i][j] == '.')
            {
                prefixY[i][j] = (i > 0 ? prefixY[i - 1][j] : 0) + 1;
            }
        }
    }
    // cout << "ok" << nl;
    for (int j = b - 1; j >= 0; j--)
    {
        for (int i = a - 1; i >= 0; i--)
        {
            if (v[i][j] == '.')
            {
                suffixY[i][j] = (i + 1 < a ? suffixY[i + 1][j] : 0) + 1;
            }
        }
    }
    // cout << "ok" << nl;
    ...
    for (int i = 0; i < a; i++)
    {
        for (int j = 0; j < b; j++)
        {
            if (v[i][j] == '.')
            {
                int now = (i > 0 ? prefixY[i - 1][j] : 0) + (j > 0 ? prefixX[i][j - 1] : 0) + (i + 1 < a ? suffixY[i + 1][j] : 0) + (j + 1 < b ? suffixX[i][j + 1] : 0);
                ans = max(ans, now);
            }
        }
    }

...
void solve()
{
    ...
    */
    ...
    int n = s.size();
    s = " "+s;
    vector<vll>v(s.size()+10,vll(2,INF));
    v[0][0] = 0;
    v[0][1] = z;
    for (int i = 1; i<=n; i++)
    {
        if (s[i]=='A')
        {
            v[i][0] = min(v[i-1][0]+y,v[i-1][1]+z+y);
            v[i][1] = min(v[i-1][0]+z+x,v[i-1][1]+x);
        }
        else
        {
            v[i][0] = min(v[i-1][0]+x,v[i-1][1]+z+x);
            v[i][1] = min(v[i-1][0]+z+y,v[i-1][1]+y);
        }
    }

    cout<<min(v[n][0],v[n][1])<<nl;

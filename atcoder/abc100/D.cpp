...
void solve()
{
    ...
    {
        cin >> arr[it].a >> arr[it].b >> arr[it].c;
        // cout <<  arr[i].a << arr[i].b << arr[i].c<<nl;
    }
    int ans = LLONG_MIN;
    for (i = -1; i < 2; i+=2)
    {
        for (j = -1; j < 2; j+=2)
        {
            for (k = -1; k < 2; k+=2)
            {
                int nowa = 0LL, nowb = 0LL, nowc = 0LL;
                sort(arr, arr + n, cmp);
                for (int it = 0; it < m; it++)
                {
                    // cout << arr[it].a << arr[it].b << arr[it].c << nl;
                    nowa += (arr[it].a);
                    nowb += (arr[it].b);
                    nowc += (arr[it].c);
                }

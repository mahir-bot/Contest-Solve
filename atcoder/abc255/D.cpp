...
void solve()
{
    ...
    while (b--)
    {
        int x;
        cin>>x;
        auto k = lower_bound(all(v),x);
        auto kk = upper_bound(all(v),x);

        if (kk == v.end())
        {

            cout<<abs((a*x)-prefix[a])<<nl;
            continue;
        }
        if (k==v.begin())
        {
        ...
        int sum = abs(prefix[pp]-(pp*x));
        // cout<<sum<<nl;
        pp = kk-v.begin();
        // cout<<pp<<nl;
        int sum2 =abs((prefix[a]-prefix[pp])-((a-pp)*x));


        cout<<sum+sum2<<nl;
        // cout<<prefix[5]-prefix[3]<<nl;
    }
